...
- **Единый производный ключ** — вывод Argon2id сразу становится ключом шифрования; хранилище открывается, только если расшифровка прошла успешно
...
| **Подмена шифротекста и неверный пароль** | AES-GCM аутентификационная метка отклоняет изменённые данные; неверный ключ не расшифровывает хранилище |
...
- **`vault.dat`** самодостаточен: `[соль 32][nonce 12][ciphertext][tag 16]`. Соль генерируется один раз при создании, хранится открытым текстом в заголовке (она не секретна) и переиспользуется при каждом перешифровании, чтобы ключ можно было вывести заново при следующем входе.
- Вывод Argon2id **и есть ключ** шифрования (без HKDF). Для входа ключ выводится из введённого пароля и предпринимается попытка расшифровать хранилище: верный пароль проверяет аутентификационную метку, неверный — провалится.
